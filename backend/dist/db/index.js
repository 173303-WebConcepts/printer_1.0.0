...
async function connectToDB() {
    try {
        const connectionInstance = await mongoose.connect(`${DB_URI}/${DB_NAME}?retryWrites=true&w=majority&appName=SF-Cluster`);
        console.log(chalk.yellow.underline.bold("Connected to the database for", `'${NODE_ENV}'`, "mode", `${connectionInstance.connection.host}`, `DB: ${DB_NAME}`));
    }
    catch (err) {
