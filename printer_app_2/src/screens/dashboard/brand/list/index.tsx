...
const BrandList = () => {
  ...
  const handleEdit = (category: any) => {
    if (category.shopType) {
      return AppToast.error("Oops", "You can't edit brand created by admin.");
    }
    ...
    <TopBottomNav>
      ...
      <ScrollView
        ...
        contentContainerStyle={{
          paddingBottom: 20, // 👈 gives actual scrollable bottom space
        }}
      >
        <View className="mt-2">
          <View className="flex-row justify-end mb-4">
            <Button variant="solid" size={"sm"} className="mt-2 w-fit" onPress={() => navigation.navigate("Add Category")}>
              ...
              <ButtonText>Add New Brand</ButtonText>
            </Button>
          </View>
        ...
        {loading ? (
          ...
          <View className="flex flex-row flex-wrap">
            {data?.brands.map((cat: any, index: any) => (
              <View
                key={cat._id || index}
          ...
          <View className="flex items-center justify-center py-10">
            <ThemedText>No Brand Found</ThemedText>
          </View>
        )}
