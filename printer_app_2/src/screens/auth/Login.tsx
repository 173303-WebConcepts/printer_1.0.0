...
const LoginSchema = Yup.object().shape({
  phone: Yup.string()
    .matches(/^03[0-9]{9}$/, "Phone number must be in format 03123456789")
...
export default function LoginScreen() {
  ...
  return (
    <View className="flex-1 justify-center">
      ...
      <View className="px-[10px] w-full lg:w-[60%] mx-auto">
        <ThemedText className="text-3xl font-grotes-bold mb-1 text-center">Sign in to your account</ThemedText>
        <ThemedText className="mb-10 text-center text-base-content-2">Welcome back! Enter phone number to login</ThemedText>

        <Formik initialValues={{ phone: "03134576103" }} validationSchema={LoginSchema} enableReinitialize onSubmit={handleLogin}>
          {({ handleChange, handleSubmit, values, errors, submitCount }) => (
            <>
              <VStack>
                ...
                {/* <TouchableOpacity className=" mt-3 self-end" onPress={() => navigation.navigate("ForgotPassword")}>
                  <ThemedText className="text-sm text-primary font-grotes-light">Forgot Password ?</ThemedText>
                </TouchableOpacity> */}
              ...
              <LoginModal showModal={showModal} setShowModal={setShowModal} phoneNumber={values.phone} />
            </>
          )}
        </Formik>
        ...
        {/* <View className="flex-row justify-center mt-4">
          <ThemedText className="text-base-content-70 mr-1">Don't have an account?</ThemedText>
          <TouchableOpacity onPress={() => navigation.navigate("Register")}>
            <ThemedText className="text-primary font-grotes-bold underline">Signup</ThemedText>
          </TouchableOpacity>
        </View> */}
      </View>
    </View>
