...
function Table() {
  ...
  const handleDelete = async ({ _id }: any) => {
    ...
    try {
      const res = await axiosInstance.delete(`/user/shopType/${_id}`);
      ...
      <div className="border-neutral bg-base-100 max-w-full rounded-md border shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <header className="flex items-center justify-start gap-5 px-5 py-4">
            <h3 className="flex gap-3 text-left font-semibold">Technology Information {isLoading != null && isLoading && <Spinner />}</h3>
            {searchKey && (
            ...
            <table className="table">
              ...
              <tbody>
                {isLoading != null && isLoading ? (
                  ...
                  data.shoptypes.map((item: any, index: any) => {
                    return (
                      <tr key={index} className="border-b-dark-2/60 border-b text-center">
                        ...
                        <td>
                          <div className={`badge badge-primary bg-primary/20 text-primary border-primary/50`}>{item.type}</div>{" "}
                        </td>
