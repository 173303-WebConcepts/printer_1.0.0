...
function Table() {
  ...
  const get = async ({ page, pageSize, ...restFilters }: any) => {
    ...
    try {
      const res = await axiosInstance.get("/category/get-business-model", {
  ...
  const handleDelete = async ({ _id }: any) => {
    ...
    try {
      const res = await axiosInstance.delete(`/category/delete-business-model/${_id}`);
  ...
  return (
    <div className="max-w-screen">
      <Add refreshCategories={() => get({ page: 1, pageSize: 10 })} />
      ...
      <div className="border-neutral bg-base-100 max-w-full rounded-md border shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <header className="flex items-center justify-start gap-5 px-5 py-4">
            <h3 className="flex gap-3 text-left font-semibold">Business Model Information {isLoading != null && isLoading && <Spinner />}</h3>
            {searchKey && (
          ...
          <div className="rounded-box border-dark-2/60 bg-base-100 max-w-full overflow-x-auto border">
            <table className="table">
              ...
              <tbody>
                ...
                ) : data && data.businessModels?.length > 0 ? (
                  data.businessModels.map((item: any, index: any) => {
                    return (
