...
function Table() {
  ...
  const handleDelete = async ({ _id }: any) => {
    ...
    try {
      const res = await axiosInstance.delete(`/brand/delete/${_id}`);
  ...
  const handleEditClick = (item: any) => {
    if (item.shopType) {
      return Toast.error("You can't edit brand created by admin");
    }
      ...
      <div className="border-neutral bg-base-100 max-w-full rounded-md border shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <header className="flex items-center justify-start gap-5 px-5 py-4">
            <h3 className="flex gap-3 text-left font-semibold">User Brand Information {isLoading != null && isLoading && <Spinner />}</h3>
            {searchKey && (
                      ...
                      <tr key={index} className="border-b-dark-2/60 border-b text-center">
                        ...
                        <td className="gap-5 p-2">
                          <div className="flex min-h-full w-full justify-center gap-5">
                            <FiEdit
                              className={`${item.shopType ? "text-primary/50" : "text-primary"} cursor-pointer`}
                              size={25}
