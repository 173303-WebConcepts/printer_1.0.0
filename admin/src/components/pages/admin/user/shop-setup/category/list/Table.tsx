...
function Table() {
  ...
  const get = async ({ page, pageSize, ...restFilters }: any) => {
    ...
    try {
      const res = await axiosInstance.get("/category/user-categories", {
        params: { page, pageSize, userId: userId ? userId[0] : "", ...cleanedFilters },
      });
  ...
  const handleDelete = async ({ _id }: any) => {
    ...
    try {
      const res = await axiosInstance.delete(`/category/delete/${_id}/${userId}`);
  ...
  const handlePageChange = (page: number) => {
   ...
   const handleEditClick = (item: any) => {
    if(item.shopType){
      return Toast.error("You can't edit category created by admin")
    }
      ...
      <div className="border-neutral bg-base-100 max-w-full rounded-md border shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <header className="flex items-center justify-start gap-5 px-5 py-4">
            <h3 className="flex gap-3 text-left font-semibold">User Category Information {isLoading != null && isLoading && <Spinner />}</h3>
            {searchKey && (
                      ...
                      <tr key={index} className="border-b-dark-2/60 border-b text-center">
                        ...
                        <td className="gap-5 p-2">
                          <div className="flex min-h-full w-full justify-center gap-5">
                            <FiEdit className={`${item.shopType ? "text-primary/50" : "text-primary"} cursor-pointer`} size={25} onClick={() => handleEditClick(item)} />
