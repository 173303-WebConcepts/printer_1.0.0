...
const AddNewCategoryModal = ({ open, setOpen, onClick, isDeleteLoading, refetchData }: any) => {
  ...
  const get = async ({ page, pageSize, ...restFilters }: any) => {
    ...
    try {
      const res = await axiosInstance.get("/category/shop-type-categories", { params: { shopType:  user?.shopType?._id} });
  ...
  const handleSubmit = async () => {
    ...
    try {
      setIsLoading(true);
      const res = await axiosInstance.put("/category/add-user-to-categories", {
        ...
        <div className="modal-box md:min-w-[800px]">
          ...
          <div className="border-b-base-content-20 border-b pb-3">
            <h3 className="flex items-center gap-2 text-lg font-bold">
              Add New Category <span className="text-primary capitalize">({user?.shopType?.type})</span>
            </h3>
            <p className="">
              Choose categories from below or{" "}
              <Link href={`/admin/user/category/list/${userId}`} className="text-primary capitalize underline">
                add a new one
              </Link>
              .
            </p>
