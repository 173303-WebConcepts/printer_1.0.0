...
function Table() {
  ...
  const get = async ({ page, pageSize, ...restFilters }: any) => {
    ...
    try {
      const res = await axiosInstance.get("/product/user-products", {
        params: { page, pageSize, userIdByAdmin: user?._id, ...cleanedFilters },
      });
  ...
  const handleEditClick = (item: any) => {
    ...
    router.push(`/admin/user/product/add/${user?._id}`);
  };
      ...
      <div className="border-neutral bg-base-100 max-w-full rounded-md border shadow-lg">
        <div className="flex flex-col md:flex-row md:items-center md:justify-between">
          <header className="flex items-center justify-start gap-5 px-5 py-4">
            <h3 className="flex gap-3 text-left font-semibold">Product Information {isLoading != null && isLoading && <Spinner />}</h3>
            {searchKey && (
              <button
                  ...
                  data.products.map((item: any, index: any) => {
                    return (
                      <tr key={index} className="border-b-dark-2/60 border-b text-center">
                        ...
                        <td>{item.tax}</td>
                        <td>
                          <p className={`badge badge-primary bg-primary/20 text-primary border-primary/50`}>{item.categoryId?.name}</p>
                        </td>
                        <td>
                          <p className={`badge badge-primary bg-primary/20 text-primary border-primary/50`}>{item.brandId?.name}</p>
                        </td>
                        <td>
                          <div>
                            <p
                              className={`badge ${item.isActive ? "badge-success bg-success/20 text-success border-success/50" : "badge-error bg-error/20 text-error border-error/50"}`}
                            >
                        ...
                        <td className="gap-5 p-2">
                          <div className="flex min-h-full w-full justify-center gap-5">
                            ...
                            <MdDeleteForever className="text-error cursor-pointer" size={25} onClick={() => handleDeleteClick(item)} />
                          </div>
                        </td>
                      </tr>
                    );
